...
void loop(void)
{
  ...
  if (comienzaOcto == true) {
    ...
    if (leerSensorNum == 0) {
      ...
      accel.getEvent(&event);
      Serial.println(event.acceleration.z);
      if (event.acceleration.y > topeAltoY || event.acceleration.y < topeBajoY
          || event.acceleration.x > topeAltoX || event.acceleration.x < topeBajoY)
      {
        nuevoRandom = true;
        cuentaPases++;
    ...
    if (leerSensorNum == 3) {
      gBrightness = 200;       fill_solid(leds, NUM_LEDS, CRGB::Green);
      controllers[3]->showLeds(gBrightness);
      Wire.beginTransmission(0x70);
      // OKs: 01 02 04 08 16 32 64 128
      Wire.write(8);
