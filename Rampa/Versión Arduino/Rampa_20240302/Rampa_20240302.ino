...
void setup() {
  ...
  for (int i = 0; i < STRING_NUM; i++) {
    pixels[i].begin();
    pixels[i].setBrightness(80); // 1/3 brightness
...
void print_sensors() {
  for (int i = 0; i < SONAR_NUM; i++) {
    Serial.print(i);
    Serial.print(":  ");
    int aux_sensor = sonar[i].ping_cm();
    Serial.print(aux_sensor);
    Serial.print("   ");
  }
  Serial.println("  ");
}
...
void rainbow(uint8_t wait) {
  ...
  for(j=0; j<256; j++) {
    for(tira=0;tira<STRING_NUM;tira++){
    for(i=0; i<pixels[tira].numPixels(); i++) {
      pixels[tira].setPixelColor(i, Wheel((i*1+j) & 255,tira));
    }
    pixels[tira].show();
  }
}
...
uint32_t Wheel(byte WheelPos, int tira) {
  if(WheelPos < 85) {
    return pixels[tira].Color(WheelPos * 3, 255 - WheelPos * 3, 0);
  } 
  else if(WheelPos < 170) {
    WheelPos -= 85;
    return pixels[tira].Color(255 - WheelPos * 3, 0, WheelPos * 3);
  } 
  else {
    WheelPos -= 170;
    return pixels[tira].Color(0, WheelPos * 3, 255 - WheelPos * 3);
  }
}
